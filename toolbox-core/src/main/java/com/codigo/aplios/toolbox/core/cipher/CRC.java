...
//
// This package has been largely inspired by Ross Williams' 1993 paper "A Painless Guide to CRC Error Detection
// Algorithms".
// A good list of parameter sets for various CRC algorithms can be found at
// http://reveng.sourceforge.net/crc-catalogue/.
/**
 * This class provides utility functions for CRC calculation using either canonical straight forward approach or using
 * "fast" table-driven implementation. Note, that even though table-driven implementation is much faster for processing
 * large amounts of data and is commonly referred as fast algorithm, sometimes it might be quicker to calculate CRC
 * using canonical algorithm then to prepare the table for table-driven implementation.
 * <p>
 */
...
public class CRC {
	...
	public static class Parameters {
		 ...
		 */
		public static final Parameters IEEE = Parameters.CRC32;

		/**
		 * Castagnoli polynomial. used in iSCSI. And also provided by hash/crc32 package.
		 */
		public static final Parameters Castagnoli =
				new Parameters(32, 0x1EDC6F41L, 0x00FFFFFFFFL, true, true, 0x00FFFFFFFFL);

		/**
		 * CRC32C is an alias to Castagnoli
		 */
		public static final Parameters CRC32C = Parameters.Castagnoli;

		/**
		 * Koopman polynomial
		 */
		public static final Parameters Koopman =
				new Parameters(32, 0x741B8CD7L, 0x00FFFFFFFFL, true, true, 0x00FFFFFFFFL);

		/**
		 * CRC64ISO is set of parameters commonly known as CRC64-ISO
		 */
		public static final Parameters CRC64ISO =
				new Parameters(64, 0x000000000000001BL, 0xFFFFFFFFFFFFFFFFL, true, true, 0xFFFFFFFFFFFFFFFFL);

		/**
		 * CRC64ECMA is set of parameters commonly known as CRC64-ECMA
		 */
		public static final Parameters CRC64ECMA =
				new Parameters(64, 0x42F0E1EBA9EA3693L, 0xFFFFFFFFFFFFFFFFL, true, true, 0xFFFFFFFFFFFFFFFFL);
	...
	/**
	 * This method implements simple straight forward bit by bit calculation. It is relatively slow for large amounts of
	 * data, but does not require any preparation steps. As a result, it might be faster in some cases then building a
	 * table required for faster calculation.
	 *
		...
		for (byte element : data) {
			...
			for (int j = 0; j < 8; j++) {
				if ((curValue & topBit) != 0)
					...
					curValue = (curValue << 1);
				System.out.printf("CRC is 0x%32X\n", curValue);
			}
	...
	/**
	 * Returns initial value for this CRC intermediate value This method is used when starting a new iterative CRC
	 * calculation (using init, update and finalCRC methods, possibly supplying data in chunks).
	 *
	...
	/**
	 * This method is used to feed data when performing iterative CRC calculation (using init, update and finalCRC
	 * methods, possibly supplying data in chunks). It can be called multiple times per CRC calculation to feed data to
	...
	public long update(long curValue, byte[] chunk, int offset, int length) {
		...
		if (this.crcParams.reflectIn)
			for (int i = 0; i < length; i++) {
				...
				curValue = this.crctable[(((byte) curValue) ^ v) & 0x00FF] ^ (curValue >>> 8);
			}
		else
			for (int i = 0; i < length; i++) {
				byte v = chunk[offset + i];
				curValue = this.crctable[((((byte) (curValue >>> (this.crcParams.width - 8))) ^ v) & 0xFF)]
						^ (curValue << 8);
			}
	...
	public CRC(Parameters crcParams) {
		...
		this.initValue = (crcParams.reflectIn) ? CRC.reflect(crcParams.init, crcParams.width)
				: crcParams.init;
		this.mask = ((crcParams.width >= 64) ? 0
				: (1L << crcParams.width)) - 1;
	...
	public static void main(String[] args) {
		...
		long ccittCrc = CRC.calculateCRC(CRC.Parameters.CRC32, data.getBytes());
		System.out.printf("CRC is 0x%16X\n", ccittCrc); // prints "CRC is 0x29B1"
