...
// assertTrue("Failed to retrieve extended computer name", Kernel32.INSTANCE
// .GetComputerNameEx(WinBase.COMPUTER_NAME_FORMAT.ComputerNameNetBIOS, buffer, lpnSize));
...
// assertTrue("Version high should be non-zero: 0x" + Integer.toHexString(version.getHigh().intValue()), version
// .getHigh().intValue() != 0);
// assertTrue("Version low should be >= 0: 0x" + Integer.toHexString(version.getLow().intValue()), version.getLow()
// .intValue() >= 0);
// }
//
...
// assertTrue("Failed to read from file", Kernel32.INSTANCE
// .ReadFile(hFile, readBuffer, readBuffer.length, lpNumberOfBytesRead, null));
//
...
//
// try {
// SymbolicLinkReparseBuffer symLinkReparseBuffer = new SymbolicLinkReparseBuffer(folder.getFileName()
// .toString(),
// folder.getFileName().toString(),
// Ntifs.SYMLINK_FLAG_RELATIVE);
//
// REPARSE_DATA_BUFFER lpBuffer =
// new REPARSE_DATA_BUFFER(WinNT.IO_REPARSE_TAG_SYMLINK, (short) 0, symLinkReparseBuffer);
//
...
//
// Kernel32.INSTANCE
// .MoveFileEx(source.getCanonicalPath(), destination.getCanonicalPath(), new DWORD(WinBase.MOVEFILE_REPLACE_EXISTING));
// assertTrue(destination.exists());
// assertFalse(source.exists());
// }
//
...
// File tmpFile =
// new File(Files.createTempFile(tmpDir, "testFindFirstFileExFindExInfoStandard", ".jna").toString());
//
// Memory p = new Memory(WIN32_FIND_DATA.sizeOf());
...
// if (false == Kernel32.INSTANCE.GetFileInformationByHandleEx(hFile, WinBase.FileBasicInfo, p, new DWORD(p
// .size()))) {
...
// if (false == Kernel32.INSTANCE.GetFileInformationByHandleEx(hFile, WinBase.FileStandardInfo, p, new DWORD(p
// .size()))) {
// fail("GetFileInformationByHandleEx failed with " + Kernel32.INSTANCE.GetLastError());
// }
...
//
// if (false == Kernel32.INSTANCE
// .SetFileInformationByHandle(hFile, WinBase.FileBasicInfo, fbi.getPointer(), new DWORD(FILE_BASIC_INFO
// .sizeOf()))) {
// fail("GetFileInformationByHandleEx failed with " + Kernel32.INSTANCE.GetLastError());
// }
//
...
//
// HANDLE hFile = Kernel32.INSTANCE
// .CreateFile(tmp.getAbsolutePath(), WinNT.GENERIC_WRITE | WinNT.DELETE, WinNT.FILE_SHARE_WRITE,
// new WinBase.SECURITY_ATTRIBUTES(), WinNT.OPEN_EXISTING, WinNT.FILE_ATTRIBUTE_NORMAL, null);
//
// assertFalse(WinBase.INVALID_HANDLE_VALUE.equals(hFile));
//
...
// if (false == Kernel32.INSTANCE.SetFileInformationByHandle(hFile, WinBase.FileDispositionInfo, fdi
// .getPointer(), new DWORD(FILE_DISPOSITION_INFO.sizeOf()))) {
...
// HANDLE processEnumHandle = Kernel32.INSTANCE
// .CreateToolhelp32Snapshot(Tlhelp32.TH32CS_SNAPALL, new WinDef.DWORD(0));
...
//
// DWORD len = Kernel32.INSTANCE
// .GetPrivateProfileString("Section", "existingKey", "DEF", buffer, new DWORD(buffer.length), tmp
// .getCanonicalPath());
// assertEquals(3, len.intValue());
// assertEquals("ABC", Native.toString(buffer));
//
// len = Kernel32.INSTANCE
// .GetPrivateProfileString("Section", "missingKey", "DEF", buffer, new DWORD(buffer.length), tmp
// .getCanonicalPath());
// assertEquals(3, len.intValue());
...
// final DWORD len = Kernel32.INSTANCE.GetPrivateProfileSection("X", buffer, new DWORD(buffer.length), tmp
// .getCanonicalPath());
//
...
// final DWORD len = Kernel32.INSTANCE.GetPrivateProfileSectionNames(buffer, new DWORD(buffer.length), tmp
// .getCanonicalPath());
...
// }
//
// final boolean result = Kernel32.INSTANCE
// .WritePrivateProfileSection("S1", "A=3\0E=Z\0\0", tmp.getCanonicalPath());
// assertTrue(result);
//
// final BufferedReader reader = new BufferedReader(new FileReader(tmp));
...
// assertEquals("GetLastError should be set to 1813", WinError.ERROR_RESOURCE_TYPE_NOT_FOUND, Kernel32.INSTANCE
// .GetLastError());
// }
//
...
// HANDLE snapshot = Kernel32.INSTANCE
// .CreateToolhelp32Snapshot(Tlhelp32.TH32CS_SNAPMODULE, new DWORD(Kernel32.INSTANCE.GetCurrentProcessId()));
// if (snapshot == null) {
...
// // check tests both
// assertTrue("The first module in the current process should be java.exe or javaw.exe", first.szModule()
// .startsWith("java"));
// assertEquals("The process ID of the module ID should be our process ID", Kernel32.INSTANCE
// .GetCurrentProcessId(), first.th32ProcessID.intValue());
...
// );
//
// assertEquals(WinBase.ES_CONTINUOUS | WinBase.ES_SYSTEM_REQUIRED | WinBase.ES_AWAYMODE_REQUIRED,
// intermediateExecutionState);
//
