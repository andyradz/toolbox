...
public class MockSecurityManager extends SecurityManager {
	...
	public MockSecurityManager() {
		...
		this.permissions.add(MockSecurityManager.setSecurityManagerPerm);
	}

	/**
	 * Define the set of permissions to be granted to classes in the o.a.c.l package, but NOT to unit-test classes in
	 * o.a.c.l.security package.
	 */
	...
	/**
	 * This returns the number of times that a check of a permission failed due to stack-walking tracing up into
	 * untrusted code. Any non-zero value indicates a bug in JCL, ie a situation where code was not correctly wrapped in
	 * an AccessController block. The result of such a bug is that signing JCL is not sufficient to allow JCL to perform
	...
	public void checkPermission(Permission p) throws SecurityException {
		...
		if (MockSecurityManager.setSecurityManagerPerm.implies(p))
		...
		for (int i = 1; i < stack.length; ++i) {
			...
			if (cname.equals("java.util.logging.Handler") && stack[i].getMethodName()
				.equals("setLevel"))
				// LOGGING CODE CAUSES ACCESSCONTROLEXCEPTION
				// http://www-01.ibm.com/support/docview.wss?uid=swg1IZ51152
				return;

			if (cname.equals("java.util.logging.Level") && stack[i].getMethodName()
				.equals("getLocalizedLevelName"))
			...
			if (cname.equals("java.security.AccessController")) {
				...
				return;
			} else if (cname.startsWith("java.") || cname.startsWith("javax.") || cname.startsWith("junit.")
					|| cname.startsWith("org.apache.tools.ant.") || cname.startsWith("sun.")) {
